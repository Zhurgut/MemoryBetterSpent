...
__global__ void kernel_smem_base(
    ...
    for (int bk = 0; bk < M/BLOCKSIZE; bk++) {
        ...
        if (out_r < N) {
            getindex(m1, BLOCKSIZE, bk_r, bk_c) = getindex_T(A, a_pitch, out_r, bk*BLOCKSIZE + bk_c);
        }
        if (out_c < S) {
...
void smem_base(torch::Tensor A, int64_t a_pitch, torch::Tensor B, torch::Tensor C, torch::Tensor X, int64_t x_pitch, torch::Tensor Y) {
    int64_t M = A.size(0);
    int64_t N = A.size(1);
    int64_t P = B.size(1);
    int64_t S = C.size(1);
    ...
    kernel_smem_base<<<nr_blocks, threads_per_block>>>(
